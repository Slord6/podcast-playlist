...
export class RSSFeedImporter {
    ...
    public async toFeed(): Promise<Feed | null> {
        ...
        return BufferedRequests.fetch(this.rssUrl).then(r => r.text()).then(rssText => {
            return parser.parseString(rssText).then(feed => {
                ...
                const feedObj = new Feed(title, this.rssUrl, new URL(feed.feedUrl as string), new URL(imgUrl), items);
                console.log(`(RSS) Feed "${title}" parsed`);
                return feedObj;
            }).catch(reason => {
